...
INGEST_CONFIG = os.path.join(
    os.environ.get("CODE_DEPENDS"), "black_pearl/dpi_ingests.yaml"
)
...
HDLR = logging.FileHandler(
    os.path.join(LOG_PATH, "black_pearl_validate_make_record.log")
)
...
def check_for_media_record(fname: str, session: requests.Session) -> tuple[str, str]:
    ...
    try:
        result = adlib.retrieve_record(
            CID_API, "media", search, "0", session, ["priref", "access_rendition.mp4"]
        )[1]
    ...
    if result:
        try:
            ...
            pass
        try:
            access_mp4 = adlib.retrieve_field_name(
                result[0], "access_rendition.largeimage"
            )[0]
        except (KeyError, IndexError):
            pass
    ...
    for file in file_list:
        ...
        if media_priref:
            ...
            if deletion_confirm and md5_match is False:
                ...
                logger.info(
                    "DELETING DUPLICATE: File has Media record, and Access Renditions populated."
                )
                try:
